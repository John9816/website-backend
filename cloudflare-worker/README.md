...
- `sys_config` CRUD
- Image generation tasks/history/public sharing with R2 storage for base64 upstream images
...
create` works. If R2 is not enabled yet, omit the `r2_buckets` binding and the
Worker will still deploy; generated base64 images are stored in D1 as
`imageData` until an R2 bucket is configured.
...
- `PUBLIC_R2_BASE_URL`: public/custom domain for R2 assets
